...
from ui_tasarim.Ui_login import Ui_Login
...
class logIn(QDialog, Ui_Login):
    def __init__(self):
        ...
        # pickle
        try:
            with open('lastlog.pc', 'rb') as file:
                log = pc.load(file)
                if len(log) > 0:
    ...
    def isReMember(self):
        if self.remember_me.isChecked():
            try:
                with open('lastlog.pc', 'wb') as file:
    ...
    def signIn(self):
        if len(self.username.text()) < 6 or len(self.password.text()) < 6:
            warning(
                "Kullanıcı Adınız veya Şifreniz 6 Karakterden Küçük Olamaz").exec_()
        else:
